...
- (void)start
{
	...
	self.listeningHandle = [[NSFileHandle alloc]
...
/*
 ACK/Completion
 ACK 90 4y FF  Return when the command is accepted
 Completion 90 5y FF  Return when the command has been executed
 */
- (void)replySuccess {
    const char reply[3] = {0x90, 0x50, 0xff};
    NSData *data = [NSData dataWithBytes:reply length:3];
    
    CFSocketError error = CFSocketSendData(self.socket, NULL, (CFDataRef) data, 0);
    NSLog(@"onSocketControllerError: %ld", (long)error);
